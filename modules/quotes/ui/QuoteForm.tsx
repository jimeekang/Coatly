...
import { findQuotePricingScopeIssues } from '@/modules/quotes/domain/quote-pricing-scopes';
import {
...
import { LineItemsSection } from '@/modules/quotes/ui/LineItemsSection';
import {
  QuoteExtraLineItems,
  toQuoteLineItemFormInput,
  type ExtraLineItemInput,
} from '@/modules/quotes/ui/QuoteExtraLineItems';
import { QuoteStatusCard } from '@/modules/quotes/ui/QuoteStatusCard';
import { ScopeBuilder } from '@/modules/quotes/ui/ScopeBuilder';
import { ClauseLibraryPicker } from '@/modules/quotes/ui/ClauseLibraryPicker';
    ...
    <div className="space-y-4">
      <section className="border-outline-variant bg-surface-container-lowest rounded-2xl border p-4 shadow-sm">
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="bg-primary-container text-on-primary-container rounded-xl px-3 py-2 text-right">
            <p className="text-on-primary-container text-xs font-medium tracking-wide uppercase">
              Estimate Total
            </p>
            <p className="text-on-primary-container mt-1 text-lg font-semibold">
              {formatAUD(total)}
            </p>
          </div>
        </div>
          ...
          {rateSetupIssues.length > 0 && (
            <div
              className={`rounded-xl border px-4 py-3 ${
                blockingRateSetupIssue
                  ? 'border-error/25 bg-error-container'
                  : 'border-warning/25 bg-warning-container'
              }`}
            >
              <p
                className={`text-xs font-semibold tracking-wide uppercase ${
                  blockingRateSetupIssue ? 'text-error' : 'text-warning'
                }`}
              >
                Rate setup check
              </p>
