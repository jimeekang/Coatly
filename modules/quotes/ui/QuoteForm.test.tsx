...
import { calculateQuoteTotals } from '@/modules/quotes/domain/quotes';
...
describe('QuoteForm', () => {
  ...
  it('uses the matching on-primary text token for estimate total cards', () => {
    render(<QuoteForm customers={[CUSTOMER]} />);

    for (const label of screen.getAllByText('Estimate Total')) {
      expect(label).toHaveClass('text-on-primary-container');
      expect(label.nextElementSibling).toHaveClass('text-on-primary-container');
    }
  });
