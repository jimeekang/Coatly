...
export function QuoteActions({
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {status === 'approved' && (
        <div className="flex flex-col gap-2.5">
          ...
          <button
            ...
            disabled={anyPending}
            className="border-outline-variant text-on-surface hover:bg-surface-container inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-2xl border bg-surface-container-lowest px-5 text-sm font-semibold transition-colors disabled:opacity-50"
          >
            {jobPending ? (
              <>
      ...
      <ConfirmDialog
        open={showDeleteModal && !hasLinkedInvoices}
        title="Delete Quote?"
        message={`${quoteNumber} will be permanently deleted. This cannot be undone.`}
        confirmLabel={deletePending ? 'Deleting…' : 'Yes, Delete Quote'}
        cancelLabel="Cancel"
        destructive
