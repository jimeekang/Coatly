...
// Structural copies of quote-pricing input contracts. Kept local so ai/domain
// stays decoupled from quotes/domain type ownership (domain modules must not
...
type QuickTrimPaintSystem = 'oil_2coat' | 'water_3coat_white_finish';
type QuickGlobalCoating =
  | 'one_coat_refresh'
  | 'two_coats_repaint'
  | 'three_coats_new_plaster';
type QuickGlobalCondition = 'good' | 'average' | 'poor';
type SelectedQuickRoom = {
  room_id: string;
...
export type AIQuotePricingCandidate = {
  ...
  global_coating?: QuickGlobalCoating;
  global_condition?: QuickGlobalCondition;
  trim_paint_system?: QuickTrimPaintSystem;
