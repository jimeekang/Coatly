...
> 기준일: 2026-06-27. 이 문서는 현재 코드 기준 외부 런칭 가능 여부와 남은 release blockers를 정리합니다.
...
다만 외부 런칭은 아직 막혀 있습니다. Supabase P0와 Vercel Preview 기본 배포 smoke는 해소됐지만, 로그인 후 실제 quote/email/invoice/job 흐름은 아직 검증되지 않았습니다. 또한 production 이메일/cron 설정과 v1 workflow replacement는 실제 painter A의 price book/quote/email 기준 end-to-end smoke가 필요합니다.
...
| -------- | --------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| P0       | Authenticated preview workflow smoke not completed        | Use a test account/fixture to smoke quote create/edit/PDF/email, public approval/reject, invoice PDF/email, approved quote to job booking, and schedule paths on the Preview deployment     |
...
Internal demo or controlled pilot is acceptable only if the tester understands that Vercel preview smoke is still required before production promotion.
