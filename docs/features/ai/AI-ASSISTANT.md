...
세션 `019e32de-aaa3-7940-aaa6-9c773d3ec251` 기준으로 AI는 새 Rate Library를 직접 계산하지 않는다. AI가 할 수 있는 일은 "이 scope는 `room_price:bedroom:walls` 또는 `prep_repair:stain_blocking` 후보일 수 있다"처럼 **rate item key 후보**를 제안하는 것까지다.

| Rate group | AI가 제안 가능 | AI 금지 |
|------------|----------------|---------|
| Average Property Prices | `apartment_2b2b` 같은 property anchor 후보, 포함 surface 설명 | min/average/high price 선택을 단정하거나 금액 생성 |
| Room Prices | bedroom/bathroom/living + selected surfaces 후보 | room full repaint를 강제하거나 walls/ceiling/trim을 잠금 |
| Base Surface Rates | walls/ceiling/trim/doors/windows/exterior surface 후보 | `/sqm`, `/lm`, `/each` rate 생성 |
| Prep & Repairs | visible/notes 기반 patching, sanding, mould, stain block 후보 | 보이지 않는 damage 또는 hidden prep 발명 |
| Access & Complexity | high ceiling, stairwell, poor access, scaffold question | access cost 또는 multiplier 생성 |
