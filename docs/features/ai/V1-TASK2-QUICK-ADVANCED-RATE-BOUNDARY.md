...
- Task 4 legacy reconstruction should use the new stale-rate fixtures as a pattern: every golden quote should assert preview/save/detail/PDF/invoice totals from the same stored snapshots.
...
Task 2 no longer blocks the v1 AI build. AI pricing candidates, Qwen draft application, and photo-assisted scope helper work may proceed only after Task 3 defines the quote form structure schema and the candidate-to-deterministic-pricing review path.
