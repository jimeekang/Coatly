...
│
├── types/
│   ├── database.ts               # Auto-generated DB types (Supabase CLI)
...
### 4. Type safety
- `types/database.ts`: auto-generated by Supabase CLI (always in sync with DB)
...
When adding columns or tables, always create a new migration file and regenerate types:
...
│
├── types/
│   ├── database.ts               # Supabase CLI 자동 생성 DB 타입
...
### 4. 타입 안전성
- `types/database.ts`: Supabase CLI로 자동 생성 (DB와 항상 동기화)
...
새 컬럼/테이블 추가 시 반드시 새 마이그레이션 파일을 만들고, `types/database.ts`를 재생성합니다:
