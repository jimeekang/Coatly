...
export default async function NewInvoicePage({
  ...
  const requestedQuoteId =
    typeof resolvedSearchParams.quoteId === 'string'
      ? resolvedSearchParams.quoteId
      : null;
  ...
  const quoteDraftResult = requestedQuoteId
    ? await getInvoiceDraftFromQuote(requestedQuoteId)
  ...
  return (
    <div className="mx-auto max-w-lg px-4 pt-4 lg:max-w-6xl">
      ...
      {pageError ? (
        ...
        <div className="border-outline-variant bg-surface-container-low rounded-2xl border border-dashed px-5 py-8">
          ...
          <Link
            href="/customers/new"
            className="bg-primary text-on-primary focus-visible:ring-primary/30 mt-4 inline-flex min-h-11 items-center rounded-xl px-4 text-sm font-semibold transition-opacity hover:opacity-90 focus-visible:ring-2 focus-visible:outline-none"
          >
            Go to New Customer
          </Link>
        </div>
